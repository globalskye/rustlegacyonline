# Rust Legacy Server Website v2.0 🎮

Полнофункциональный веб-сайт для сервера Rust Legacy с **базой данных PostgreSQL**, поддержкой двух языков (русский и английский), REST API и современным дизайном.

## 🚀 Что нового в v2.0

### ✨ Основные улучшения:
- **PostgreSQL база данных** для хранения всего контента
- **REST API** для управления данными
- **Система новостей** с возможностью публикации
- **Расширяемая архитектура** - легко добавлять новые функции
- **Автоматическая миграция** схемы базы данных
- **Начальные данные** загружаются автоматически

### 🗄️ Что хранится в базе данных:
- Информация о сервере (название, макс. игроков, версия, ссылка на скачивание)
- Описания на разных языках
- Особенности сервера (features)
- Новости с датами публикации
- Данные игроков (для будущего функционала)
- Настройки системы

## 🏗️ Архитектура проекта

```
rustlegacyonline/
├── backend/                 # Go API сервер
│   ├── handlers/          # HTTP обработчики (по доменам)
│   │   ├── serverinfo.go, features.go, news.go, shop.go, ...
│   │   └── health.go
│   ├── routes/            # Регистрация маршрутов
│   ├── models/            # Модели данных (GORM)
│   ├── database/          # Подключение, миграции, seed
│   ├── pkg/gameserver/    # Source engine A2S_INFO query
│   └── main.go
├── frontend/               # React приложение
│   └── src/
│       ├── services/      # API клиент
│       ├── components/    # React компоненты
│       └── i18n/         # Переводы
├── deploy/                 # Production deployment
│   ├── docker-compose.production.yml
│   └── DEPLOY.md          # Инструкция по деплою на rustlegacy.online
└── docker-compose.yml     # Локальная разработка
```

## 📊 API Endpoints

### Server Info
- `GET /api/server-info` - Получить информацию о сервере
- `PUT /api/server-info` - Обновить информацию

### Features
- `GET /api/features?lang=ru` - Получить особенности сервера
- `POST /api/features` - Создать новую особенность
- `PUT /api/features/{id}` - Обновить особенность
- `DELETE /api/features/{id}` - Удалить особенность

### News
- `GET /api/news?lang=ru&published=true` - Получить новости
- `GET /api/news/{id}` - Получить конкретную новость
- `POST /api/news` - Создать новость
- `PUT /api/news/{id}` - Обновить новость
- `DELETE /api/news/{id}` - Удалить новость

### Players
- `GET /api/players?online=true` - Получить список игроков
- `GET /api/players/{steamid}` - Получить данные игрока

### Health
- `GET /api/health` - Проверка статуса сервера

## 🚀 Быстрый старт

### Вариант 1: Docker (Рекомендуется)

```bash
# Запустить все сервисы (PostgreSQL + Backend + Frontend)
docker-compose up --build

# Сайт будет доступен на http://localhost:3000
# API на http://localhost:8080
# PostgreSQL на localhost:5432
```

#### Сброс базы данных (полная перезаливка seed)

```powershell
# Windows
.\scripts\reset-db.ps1
```

```bash
# Linux / Mac
./scripts/reset-db.sh
```

Или вручную (сначала останови backend, иначе будет ошибка «cannot drop the currently open database»):
```bash
docker stop rustlegacy-backend
docker exec rustlegacy-postgres psql -U rustlegacy -d postgres -c "DROP DATABASE IF EXISTS rustlegacy;"
docker exec rustlegacy-postgres psql -U rustlegacy -d postgres -c "CREATE DATABASE rustlegacy;"
docker start rustlegacy-backend
```

### Вариант 2: Локальная разработка

#### 1. Запустите PostgreSQL

```bash
docker run --name rust-postgres \
  -e POSTGRES_DB=rustlegacy \
  -e POSTGRES_USER=rustlegacy \
  -e POSTGRES_PASSWORD=rustlegacy_password \
  -p 5432:5432 \
  -d postgres:15-alpine
```

#### 2. Запустите Backend

```bash
cd backend
go mod download
go run main.go
```

#### 3. Запустите Frontend

```bash
cd frontend
npm install
npm start
```

## 📝 Примеры использования API

### Создание новости

```bash
curl -X POST http://localhost:8080/api/news \
  -H "Content-Type: application/json" \
  -d '{
    "language": "ru",
    "title": "Обновление сервера",
    "content": "Сегодня мы добавили новые функции!",
    "published": true,
    "publishedAt": "2025-01-27T12:00:00Z"
  }'
```

### Обновление информации о сервере

```bash
curl -X PUT http://localhost:8080/api/server-info \
  -H "Content-Type: application/json" \
  -d '{
    "id": 1,
    "name": "My Rust Legacy Server",
    "maxPlayers": 150,
    "gameVersion": "Legacy",
    "downloadUrl": "https://mysite.com/download"
  }'
```

### Добавление новой особенности

```bash
curl -X POST http://localhost:8080/api/features \
  -H "Content-Type: application/json" \
  -d '{
    "serverInfoId": 1,
    "language": "ru",
    "title": "Кастомные плагины",
    "description": "Уникальные плагины для улучшения геймплея",
    "icon": "zap",
    "order": 5
  }'
```

## 🔧 Настройка и кастомизация

### Изменение цветовой схемы

Отредактируйте `frontend/src/App.css`:

```css
:root {
  --rust-orange: #d4532f;
  --rust-dark: #1a1612;
  /* ... другие цвета */
}
```

### Добавление нового поля в модель

1. Обновите модель в `backend/models/models.go`
2. База данных автоматически мигрирует при следующем запуске

### Переменные окружения

Backend поддерживает следующие переменные:

```env
PORT=8080
DB_HOST=postgres
DB_PORT=5432
DB_USER=rustlegacy
DB_PASSWORD=rustlegacy_password
DB_NAME=rustlegacy
```

Frontend:
```env
REACT_APP_API_URL=http://localhost:8080/api
```

## 📦 База данных

### Схема PostgreSQL

**ServerInfo** - Информация о сервере
**Description** - Переводы описаний
**Feature** - Особенности сервера
**News** - Новости
**Player** - Данные игроков
**Setting** - Настройки системы

### Подключение к БД

```bash
# Через Docker
docker exec -it rustlegacy-postgres psql -U rustlegacy -d rustlegacy

# Локально
psql -h localhost -U rustlegacy -d rustlegacy
```

### Полезные SQL команды

```sql
-- Посмотреть все новости
SELECT * FROM news;

-- Посмотреть особенности на русском
SELECT * FROM features WHERE language = 'ru';

-- Обновить макс. игроков
UPDATE server_infos SET max_players = 200 WHERE id = 1;
```

## 🎯 Расширение функционала

### Добавление новой API endpoint

1. Создайте handler в `backend/handlers/handlers.go`
2. Зарегистрируйте route в `backend/main.go`
3. Добавьте метод в `frontend/src/services/api.ts`

### Пример: Добавление системы достижений

```go
// backend/models/models.go
type Achievement struct {
    ID          uint      `gorm:"primaryKey"`
    Name        string
    Description string
    Icon        string
    Points      int
    CreatedAt   time.Time
}

// backend/handlers/handlers.go
func GetAchievements(w http.ResponseWriter, r *http.Request) {
    var achievements []models.Achievement
    database.DB.Find(&achievements)
    json.NewEncoder(w).Encode(achievements)
}

// backend/main.go
api.HandleFunc("/achievements", handlers.GetAchievements).Methods("GET")
```

## 🛠️ Технологический стек

### Backend
- **Go 1.21** - Язык программирования
- **GORM** - ORM для PostgreSQL
- **Gorilla Mux** - HTTP роутер
- **PostgreSQL 15** - База данных

### Frontend
- **React 18** - UI библиотека
- **TypeScript** - Типизация
- **Framer Motion** - Анимации
- **i18next** - Мультиязычность
- **Lucide React** - Иконки

### DevOps
- **Docker** - Контейнеризация
- **Docker Compose** - Оркестрация
- **Nginx** - Веб-сервер

## 📱 Адаптивный дизайн

Сайт полностью адаптирован для:
- 📱 Мобильных телефонов (320px+)
- 📱 Планшетов (768px+)
- 💻 Десктопов (1024px+)
- 🖥️ Больших экранов (1920px+)

## 🔐 Безопасность

Для продакшена рекомендуется:
- Изменить пароли БД
- Настроить HTTPS
- Добавить rate limiting
- Настроить CORS для конкретных доменов
- Добавить аутентификацию для админ-панели

## 📈 Мониторинг

```bash
# Логи backend
docker logs rustlegacy-backend -f

# Логи БД
docker logs rustlegacy-postgres -f

# Статус всех сервисов
docker-compose ps
```

## 🤝 Вклад в разработку

Структура проекта разработана для легкого расширения:
- Добавляйте новые модели в `models/`
- Добавляйте обработчики в `handlers/`
- Создавайте новые компоненты в `components/`
- Расширяйте API в `services/api.ts`

## 📄 Лицензия

MIT License

---

Создано с ❤️ для сообщества Rust Legacy
