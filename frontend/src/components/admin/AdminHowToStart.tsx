...
export default function AdminHowToStart({ onMessage }: { onMessage: (t: string, type: 'success' | 'error') => void }) {
  ...
  useEffect(() => {
    apiService.getHowToStartSteps().then(s => setItems((s || []).sort((a, b) => a.stepNumber - b.stepNumber))).catch(() => onMessage('Failed', 'error')).finally(() => setLoading(false));
  }, []);
