...
const emptyForm: { serverId: number; language: 'en' | 'ru'; name: string; description: string; order: number } = { serverId: 0, language: 'en', name: '', description: '', order: 1 };
...
export default function AdminPlugins({ onMessage }: { onMessage: (t: string, type: 'success' | 'error') => void }) {
  ...
  const edit = (i: Types.Plugin) => {
    setEditing(i);
    setForm({ serverId: i.serverId ?? 0, language: i.language as 'en' | 'ru', name: i.name, description: i.description || '', order: i.order });
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {items.map(i => (
          <div key={i.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'var(--bg-darker)', borderRadius: 8 }}>
            <span>{i.name} ({i.language}) - {i.commands?.length || 0} commands {i.serverId ? `[Server ${i.serverId}]` : '[All]'}</span>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
